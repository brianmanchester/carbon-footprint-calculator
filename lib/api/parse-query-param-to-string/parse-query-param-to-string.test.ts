...
test('parseQueryParamToString to return param values when there is a match', () => {
...
test('parseQueryParamToString to return an error when there is no match', () => {
  expect(parseQueryParamToString({ good: 'test' }, 'bad').error).toBeTruthy;
});

test('parseQueryParamToString to return an error when the value is not a string', () => {
